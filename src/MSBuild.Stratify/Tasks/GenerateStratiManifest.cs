...
{
    public class GenerateStratiManifest : BaseTask
    {
        ...
        [Output]
        public string StratiUniqueName { get; set; }
        ...
        public override bool ExecuteTask()
        {
            ...
            CreatedStratiManifestPath = Path.Combine(NuspecOutputPath, "strati.manifest");

            StratiUniqueName = ManifestTools.GenerateManifestUniqueName(PackageId);

            stratiManifest.PackageId.Value = PackageId;
            stratiManifest.Version.Value = PackageVersion;
            stratiManifest.UniqueName.Value = StratiUniqueName;
            ...
            foreach (ITaskItem item in ProjectReferenceStrati)
            {
                ...
                {
                    case "solution":
                        stratiManifest.ProcessSolution(item, OverwriteUnmanaged, PublishAndActivate);
                        break;
                    case "configdata":
    ...
    public static class GenerateStratiManifestExtensions
    {


        public static void ProcessSolution (this StratiManifestXDocument manifest, ITaskItem solutionItem, bool overwriteUnmanaged, bool pubishAndActivate)
        {
            ...
            {
                solution.OverwriteUnmanaged.Value = "true";
            }
            else if (!String.IsNullOrEmpty(solutionItem.GetMetadata("overwriteunmanagedcustomizations")) )
            {
                if (Boolean.TryParse(solutionItem.GetMetadata("overwriteunmanagedcustomizations"), out bool owParseResult))
                {
                    if (owParseResult)
                    {
                        solution.OverwriteUnmanaged.Value = "true";
            ...
            if (pubishAndActivate)
            {
                solution.PublishAndActivate.Value = "true";
            }
            else if (!String.IsNullOrEmpty(solutionItem.GetMetadata("publishworkflowsandactivateplugins")))
            {
                if (Boolean.TryParse(solutionItem.GetMetadata("publishworkflowsandactivateplugins"), out bool paParseResult))
                {
                    if (paParseResult)
                    {
                        solution.PublishAndActivate.Value = "true";
